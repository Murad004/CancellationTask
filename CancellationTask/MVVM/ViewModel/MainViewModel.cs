using CancellationTask.Command;
using CancellationTask.Encryption;
using CancellationTask.Model;
...
namespace CancellationTask.MVVM.ViewModel
{
                            ...
                            if (i == count - 1)
                            {
                                ...
                                for (int j = 0; j < 101; j++)
                                {
                                    ...
                                    if (Value == 100)
                                    {
                                        try
                                        {
                                            w = EncryptHelper.EString(passwordtext, Document.GetText());

                                            EncryptHelper.FileStreamWrite(w, path);
                                            EncryptHelper.FileStreamRead(w, path);
                                    ...
                                    if (Value != 100)
                                    {
                                        try
                                        {
                                            ...
                                            w = EncryptHelper.EString(passwordtext, Document.GetText().Substring(0, Convert.ToInt32(100 * (j++))));
                            ...
                            if (i == count - 1)
                            {
                                ...
                                for (int j = 0; j < 101; j++)
                                {
                                    ...
                                    if (Value == 100)
                                    {
                                        try
                                        {
                                            w2 = EncryptHelper.DString(passwordtext, EncryptHelper.EString(passwordtext, Document.GetText()));

                                            EncryptHelper.FileStreamWrite(w2, path);
                                            EncryptHelper.FileStreamRead(w2, path);
                                    ...
                                    if (Value != 100)
                                    {
                                        try
                                        {
                                            ...
                                            w2 = EncryptHelper.DString(passwordtext, EncryptHelper.EString(passwordtext, Document.GetText().Substring(0, Convert.ToInt32(100 * (j++)))));
